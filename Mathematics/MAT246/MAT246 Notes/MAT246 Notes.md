...
# Table of Contents
[[MAT246 Textbook.pdf]]  
[[MAT246 Lin Alg Done Right Textbook.pdf]]
...
> This material is very similar to [[MAT224 Notes]], possible reference for help

### Vector Space
A [[Vector Space]] is a [[Set]] $V$ with two operations defined for its elements
- Vector Addition
...
The symbol $V$ is for any [[Vector Space]], and $F$  here is for the set of all [[Scalar|Scalars]]

### Subspaces
A [[Subspace]] is a [[Subset]] of a [[Vector Space]] which still retains all the properties of a [[Vector Space]]  
Properties
- The union of two subspaces is not necessarily a subspace

Although that is its definition, and we can check if a subset is a [[Vector Space]] itself to verify that it is a [[Subspace]], we can also check that the following three conditions hold 
...
### Span & Bases
A [[Vector]] $v\in V$ [[Linear Combination]] of a set of vectors $v_{1},...,v_{m}$ if there exists [[Scalar|Scalars]] $a_{1},...,a_{m}\in \mathbb{F}$ where:  
	$v=a_{1}v_{1}+a_{2}+v_{2}+...+a_{m}v_{m}$

A [[Span]] (Also called a **Linear Span**) is defined as:  
...
> A vector $v$ is a [[Linear Combination]] of another set of vectors, if its possible to add those vectors together with different scalars to get $v$  
> A [[Span]] is the set of all the [[Linear Combination|Linear Combinations]] from a set of [[Vector|Vectors]]

### Linear Independence
A set of vectors $v_{1},...,v_{m}$ is [[Linear Independence|Linearly Independent]] if the only solution for $a_{1},...,a_{m}\in \mathbb{F}$ to the equation:  
	$a_{1}v_{1}+...+a_{m}v_{m}=0$  
Is $a_{1}=...=a_{m}=0$

> So, a set of vectors is [[Linear Independence|Linearly Independent]] if there is only one [[Linear Combination]] of them which results in 0, and that is, when all the scalars are 0  
...
> 	Thus, for a set of vectors to be [[Linear Independence|Linearly Independent]], they must not have any two vectors which "lie on the same path"

A set of vectors $v_{1},...,v_{m}$ is [[Linear Dependence|Linearly Dependent]] if there exists $a_{1},...,a_{m}\in \mathbb{F}$ which are not all zero such that:  
	$a_{1}v_{1}+...+a_{m}v_{m}=0$  
	A set being Linearly Dependent is just a set of vectors is not Linearly Independent

### Bases
A [[Base (Basis)|Basis]] of a [[Finite Dimensional]] [[Vector Space]] is a [[Span|Spanning List]] that is [[Linear Independence|Linearly Independent]]  
**Properties**
...
[[Dimension]] of a [[Vector Space]] $V$ is the number of [[Base (Basis)|Bases]] in any given basis of $V$  
**Properties**  
...
For a typical vector space $V=\mathbb{F}^{n}$ (over $\mathbb{F}$) and $u=(u_{1},...,u_{n}), v=(v_{1},...,v_{n})\in \mathbb{F}^{n}$, an [[Inner Product]] can be defined as:  
$$\langle u,v \rangle = \sum\limits_{i=1}^{n}u_{i}\overline{v}_{i}$$  
	For $\mathbb{F}=\mathbb{R}$, this is the usual [[Dot Product]]

An [[Inner Product Space]] is a Vector Space with an [[Inner Product]] $\langle .,.\rangle$ 

> So, the Inner Product is the *formal definition* of the [[Dot Product]].  
...
The [[Norm]] of a [[Vector]] in a [[Inner Product Space]] represents like the "length" or "magnitude" of a vector in $\mathbb{R^{n}}$  
A [[Norm]] on $V$ over $F$ is a [[Map]]  
...
The [[Dual Basis]] of $v_{1},...,v_{n}$ is the list $\varphi_{1},...,\varphi_{n}$ of elements in $V'$ where each $\varphi_{j}$ is the [[Linear Functional]] on $V$ such that  
...
For any $U\subseteq V$, the [[Annihilator]] of $U$, denoted $U^{0}$ is:  
...
We can use the [[Annihilator]] to define the [[Null Space]] of a [[Dual Map]]
...
> 		#1, 3, 5, 9, 10, 12, 13  
> [[Week 11 Solutions.pdf]]

The [[Product of Vector Spaces]] $V_{1}\times \dots \times V_{m}$ is defined by  
...
- We can define a linear map $\Gamma: V_{1}\times \dots \times V_{m}\rightarrow V_{1}+\dots+V_{m}$  with $\Gamma(v_{1},\dots, v_{m})=v_{1}+\dots+v_{m}$. Then $V_{1}+\dots+V_{m}$ is a [[Direct Sum]] if and only if  $\Gamma$ is [[Injective]]

A [[Bilinear Functional]] on $V\times W$ is a function $\beta:V\times W\rightarrow F$ such that:
...
> Simplification:  
> 	The Tensor of two vector spaces, is the vector space of bilinear functionals, of the [[Dual Space|Dual Spaces]] of the two vector spaces  
...
An [[M-Linear Functional]] on $V_{1}\times \dots \times V_{m}$ is a function $\beta:V_{1}\times \dots \times V_{m}\rightarrow \mathbb{F}$, that is a [[Linear Functional]] in each slot when the other slots are held fixed  
Properties
...
Now with this definition, we can define the [[Tensor]] Product $V_{1}\otimes\dots\otimes V_{m}$ as $B(V_{1}',\dots,V_{m}')$, where for $v\in V_{1},\dots,v_{m}\in V_{m}$, $v_{1}\otimes\dots\otimes v_{m}$ is the element of $V_{1}\otimes\dots\otimes V_{m}$  
