...
[[STA302 Sheather Textbook.pdf]]
[[STA302 Rencher Textbook.pdf]]
[[STA302 Cheat Sheet]]
...
> Slides: 
> 	[[STA302 Module 1.pdf]]
> 	[[STA302 Katherine Module 1 Large Slides.pdf]]
> Practice: [[STA302 Module 1 Practice.pdf]]
> R: [[module1.r]]
> Topic: Simple Linear Regression Models and Basics

Recall
- [[Random Variable|Random Variables]] and [[Probability Distribution|Probability Distributions]]
- [[Expected Value]]
- [[Variance]]
- [[Independence]]
...
A [[Simple Linear Regression Model]] is the formula:
...
> 	![[Pasted image 20241009092021.png]]
...
- Where $\hat \beta_{0}$ and $\hat\beta_{1}$ *estimate* the unknown parameters of the [[Population Regression Model]] $\beta_{0},\beta_{1}$
...
We can calculate the error, or [[Residual]] of a point in our **Fitted Regression Model**, compared to our observed dataset with: 
$$e=y_{i}-(\hat \beta_{0}+\hat\beta_{1}x_{i})$$

The [[Residual Sum of Squares]], is the sum of all the errors/[[Residual|Residuals]] of our [[Fitted Regression Model]] squared
$$RSS=RSS(b_{0},b_{1})=\sum\limits_{i=1}^{n}[y_{i}-(b_{0}+b_{1}x_{i})]^{2}$$
	$b_{0},b_{1}$ are our estimated parameters of our [[Fitted Regression Model]]
	$x_{i},y_{i}$ is a real observation of our dataset

The [[Method of Least Squares]] is a method to find $\beta_{0}$ and $\beta_{1}$ for a fitted linear regression model for a dataset 
	The method, is finding the minimizers to the [[Residual Sum of Squares]]
...
> 	![[STA302 Notes Method of Least Squares Image.png]]

For a [[Simple Linear Regression Model]] with two variables, the result will be
...
> Formulas for [[Covariance]] and [[Variance]] may help
...
> Slides: 
> 	[[STA302 Module 2.pdf]]
> 	[[STA302 Katherine Module 2 Large Slides.pdf]]
> Practice: [[STA302 Module 2 Practice.pdf]]
> R:
> 	[[module2.r]]
> 	[[module2_plot.r]]

### Simple Linear Regression
The [[Simple Linear Regression Model]] can be written in matrix form as:
...
>Matrix form exists, so that given a vector of $x$, we can plug it into the [[Simple Linear Regression Model]] and get a vector $Y$ of the results
>	It describes calculating the entire set of data at once

Our eqations from 

### Multiple Linear Regression
> The [[Multiple Linear Regression Model]] is the [[Linear Regression Model]] with multiple **Explanatory Variables** which describe one **Response Variable**

The formula for a [[Multiple Linear Regression Model]] is generally:
...
- $Var(Y|X=x)=Var(Y|X_{}1=x_{1},...,X_{p}=x_{p})=\sigma^{2}$
 ...
 The [[Multiple Linear Regression Model]] can also be written as:
...
> I think whenever they do $x_{i}^{T}$, or anything like that, they are saying that so that the vector is in the correct orientation, to make [[Matrix Multiplication]] possible
...
>Again, matrix form exists, so that given a vector of $x$, we can plug it into the [[Simple Linear Regression Model]] and get a vector $Y$ of the results
...
Choosing a vector $b^{*}$ of dimension $p+1$, the [[Residual]] of a [[Multiple Linear Regression Model]] given $y,x$ is
$$e=y_{i}-x_{i}^{T}b^{*}$$

The vector of all residuals is denoted:
$$E=Y-Xb^{*}$$

An the [[Residual Sum of Squares]] is:
$$\sum\limits_{i=1}^{n}(y_{i}-x_{i}^{T}b)^{2}=\sum\limits_{i=1}^{n}(y_{i}-(b_{0}x_{i,1}+\dots+b_{p}x_{i,p}))^{2} =(y-Xb)^{T}(y-Xb)$$

The [[Method of Least Squares]] works similarly here, but now uses matrix operations.
...
> ![[STA302 Notes Multiple Linear Regression Method.png]]
...
> Slides: 
> 	[[STA302 Module 3.pdf]]
> 	[[STA302 Katherine Module 3 Large Slides.pdf]]
> Practice: [[STA302 Module 3 Practice.pdf]]
> R: [[module3.rmd]]
...
For this week
- Always assume [[Multiple Linear Regression Model]]
- $\hat \beta, \hat\sigma^{2}$ are both [[Estimator|Estimators]]
...
[[Sampling Distribution]] for a [[Residual]] of [[Multiple Linear Regression Model]] satisfies:
- $E(\hat e_{i}|X)=0$
- $Var(\hat e_{i}|X)=\sigma^{2}(1-h_{i,i})$
	- $h_{i,i}$ are entries of [[Hat Matrix]]: $H=X(X^{T}X)^{-1}X^{T}$
- If errors $e_{i}$ are normally distributed, residuals are normally distributed

The [[Residual]] of a [[Multiple Linear Regression Model]] is random
- The [[Sampling Distribution]] for a residual $\hat e_{i}$ satisfies
	- $E(\hat e_{i}|X)=0$
	- $Var(\hat e_{i}|X)=\sigma^{2}(1-h_{i,i})$
		- $h_{i,i}$ are entries of [[Hat Matrix]]: $H=X(X^{T}X)^{-1}X^{T}$
	- If errors $e_{i}$ are normally distributed, residuals are normally distributed

The [[Method of Least Squares|Least Squares]] solution $\hat b_{0}, \hat b_{1}$ calculated for  a given $Y_{1},...,Y_{n}$ is **random**
- The distribution of $\hat b_{0}, \hat b_{1}$ follows a [[Sampling Distribution]]
...
- Independent and normal errors
	...
	- Residuals/errors are [[Independent Identically Distributed Random Variables]] of $N(0,\sigma^{2})$
...
Recall that the [[Sampling Distribution]] for a [[Residual]] $\hat e_{i}$ satisfies
- $E(\hat e_{i}|X)=0$
- $Var(\hat e_{i}|X)=\sigma^{2}(1-h_{i,i})$
	- $h_{i,i}$ are entries of [[Hat Matrix]]: $H=X(X^{T}X)^{-1}X^{T}$
- If errors $e_{i}$ are normally distributed, residuals are normally distributed
...
A [[Standardized Residual]] is a [[Residual]] which is transformed so that they are approximately $N(0,1)$
...
> 	![[STA302 Notes Good Residual Plot Image.png|425]]
> Example of Bad Residual Plots
> 	![[STA302 Notes Bad Residual Plot Image.png|246]]![[STA302 Notes Bad Residual Plots.png|254]]
> Example of Good and Bad QQ Plots
> 	![[STA302 Notes Good and Bad QQ Plots Image.png|234]]
> Example of good Response versus Fitted Values, Pairwise Scatterplots
> 	![[STA302 Notes Response vs Fitted Values.png]]

## Week 4
> Slides: 
> 	[[STA302 Module 4.pdf]]
> 	[[STA302 Katherine Module 4 Large Slides.pdf]]
> Practice: [[STA302 Module 4 Practice.pdf]]
> R: 
> 	[[module4.r]]
> 	[[module4_activity_template.rmd]]
...
If a violation of *normality* is observed, we can use the [[Box-Cox Method]]
The [[Box-Cox Method]] is a method that picks the best **Power Transformation** for a predictor or the response which approximates normality
...
## Week 5
> Slides: 
> 	[[STA302 Module 5 Slides.pdf]]
> 	[[STA302 Katherine Module 5 Large Slides.pdf]]
> Practice: [[STA302 Module 5 Practice.pdf]]
> R: [[module5_activity_template.rmd]]
...
Using estimated variance now introduces more variation though, so we compare our distribution with the [[T-Distribution]], where:
$$\frac{\hat\beta-\beta}{\sqrt{\sigma^{2}(X^{T}X)^{-1}}}\sim T_{n-p-1}$$

With this sampling distribution, we can now construct a $(1-a)$% [[Confidence Interval]] for each predictor with:
...
## Week 7
> Slides:
> 	[[STA302 Module 6 Slides.pdf]]
> 	[[STA302 Katherine Module 6 Large Slides.pdf]]
> Practice: [[STA302 Module 6 Practice.pdf]]
> R: [[module6.r]]


## Week 8
> Slides: 
> 	[[STA302 Module 7 Slides.pdf]]
> 	[[STA302 Katherine Module 7 Large Slides.pdf]]
> Practice: [[STA302 Module 7 Practice.pdf]]
> R:
> 	[[module7.r]]
> 	[[module7_activity.rmd]]
...
## Week 10
> Slides:
> 	[[STA302 Module 8 Slides.pdf]]
> 	[[STA302 Katherine Module 8 Large Slides.pdf]]
> Practice: [[STA302 Module 8 Practice.pdf]]
> R:
> 	[[module8.r]]
...
## Week 11
> Slides:
> 	[[STA302 Module 9 Slides.pdf]]
> 	[[STA302 Katherine Module 9 Large Slides.pdf]]
> R: [[module9.r]]

## Week 12
> Slides:
> 	[[STA302 Module 10 Slides.pdf]]
> 	[[STA302 Katherine Module 10 Large Slides.pdf]]
> Practice: [[STA302 Module 10 Practice.pdf]]
> R: [[module10.r]]
